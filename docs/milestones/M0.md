...
- The Multiboot2 header is how GRUB recognizes the kernel and proceeds to load its ELF segments into memory at their specified addresses, which come from the ELF program headers produced by the linker script.
- The early entry point can be minimal as long as the stack is valid and the CPU doesn’t fall off into random memory.
